...
from app.application.dtos.factura_dto import EmitirFacturaRequest, EmitirFacturaResponse
...
class EmitirFacturaUseCase(ComprobanteService):
    def __init__(
        self,
    ...
    async def emitir_factura(self, dto: EmitirFacturaRequest) -> EmitirFacturaResponse:
        ruc = Ruc(dto.ruc_emisor)
        ruc_receptor = Ruc(dto.ruc_receptor)
        ...
        factura = Factura(
            ...
            razon_social_receptor=dto.razon_social_receptor,
            tipo_doc_cliente=dto.tipo_doc_cliente,
            num_doc_cliente=dto.num_doc_cliente,
            nombre_cliente=dto.nombre_cliente,
        )

        ComprobanteDomainService.validar_montos(factura)

        totales = ComprobanteDomainService.calcular_totales(items)
        xml_sin_firmar = self._ubl_builder.build_factura(
            factura, totales["subtotal"], totales["igv"], totales["total"]
        )
        ...
        filename = f"{dto.ruc_emisor}-{factura.tipo.value}-{dto.serie}-{dto.correlativo:08d}"
    ...
    async def emitir_boleta(self, dto: object) -> object:
        raise NotImplementedError("Usar EmitirBoletaUseCase")
